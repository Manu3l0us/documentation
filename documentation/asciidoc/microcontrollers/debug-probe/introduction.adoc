...
WARNING: If your target is not supplied by the same computer as the Raspberry Pi Debug Probe, care should be taken to either disconnect the target power supply before connecting or to first connect the GND wire and only then the I/O lines (RX, TX, SC, SD). Otherwise, the potential differences can destroy the Debug Probe.
